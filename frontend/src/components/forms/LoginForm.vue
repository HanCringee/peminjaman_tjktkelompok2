...
async function handleLogin() {
  ...
  try {
    await authStore.login(form.nik, form.password)
    showSuccess('Login berhasil')
    const redirect = router.currentRoute.value.query.redirect || '/dashboard'
    router.push(redirect)
  } catch (err) {
    if (err.response?.status === 401) {
      showError(err.response.data?.message || 'NIS atau Password salah')
    } else {
...
<template>
  <div class="max-w-md mx-auto w-full">
    <div class="bg-surface rounded-xl shadow-lg border border-muted/20 p-8">
      <h2 class="text-2xl font-bold text-text mb-6 text-center">Login</h2>
      ...
      <p class="text-center text-sm text-muted mt-6">
        Belum punya akun?
        <router-link to="/register" class="text-accent hover:text-muted font-medium">
          Register di sini
        </router-link>
      </p>
    </div>
